...
`sticky` + `100dvh` 조합을 걷어내고, **lg 이상에서 화면 전체를 세로 flex 셸**로 만든다.

```
lg 이상:
  최상위        h-[100dvh] flex flex-col overflow-hidden
...
넓은 화면에서 너무 늘어지지 않도록 셸에 상한을 둔다: `lg:max-w-[1600px] lg:mx-auto`.
상단 블록과 2단이 **같은 좌우 정렬선**을 갖는 것이 이 항목의 합격 조건이다.
